...
function sendBroadcast() {
  for (var i = 0; i< 100; i++) {
  	  // broadcast is built into the test harness - this will fail locally
      broadcast();
  }
}
...
// Set the message up so we can test it
var message = "th1s 1s 4 t3st. th1s 1s 0nl5 4 t3st. 1f th1s w3r3 4 r34l m3ss4g3, 502 w021d g3t s0m3th1ng m34n1ngf2l."

// Output the unmodified message
console.log(message);

// Function to decode the message - have to do the 21 first so 'would' will get decoded first
// NOTE: for this to run locally you have to remove 'message' - only works in the test harness
function decodeMessage(message) {
	message = message.replace(/0/g,"o");
	message = message.replace(/21/g,"ul");
	...
	// Return the message as that's what the tas
	return(message);
}
...
// Send the decoded message to the console for manual check
console.log(message);
...
function returnToEarth() {
  ...
  // Call broadcast
  var returnX = decodeMessage(broadcast("x"));
  var returnY = decodeMessage(broadcast("y"));
  var returnZ = decodeMessage(broadcast("z"));
  
  // Convert HEX to Int
  returnX = parseInt(returnX,16);
  returnY = parseInt(returnY,16);
  returnZ = parseInt(returnZ,16);
  
  // Set the navigation objects parameters
  navigation.x = returnX;
  navigation.y = returnY;
  navigation.z = returnZ;
}
